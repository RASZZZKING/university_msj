...
export async function POST(request) {
  ...
  let savedFiles = {};
  if(foto){
    if (!["image/jpg", "image/jpeg", "image/png"].includes(foto.type)) {
      return new Response(
    ...
    const filePath = path.join(
      ...
      "public/foto_siswa",
      fileName
    ); // Simpan file di folder non-public
  ...
  const createFullData = await prisma.data_calon_siswa.create({ 
    data: {
