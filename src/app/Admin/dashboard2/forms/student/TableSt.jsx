...
const TableSt = ({ datax, field, forMap }) => {
  //outer
          ...
          <tbody className="text-color-dark  opacity-90">
            ...
            {datax
              ...
              .map((cb, i) => {
                const showImg = (isi) => {
                    setIsVisible(true)
                  setUrl(`/foto_siswa/${isi}`);
                };
                const handleAccept = async (e) => {
                  e.preventDefault();
                  const id_user = cb.data_locs.id_user;
                  ...
                  const response = await fetch("/api/v1/dataSiswa/accept", {
                    method: "POST",
                ...
                const handleReject = async (e) => {
                  ...
                  const response = await fetch("/api/v1/dataSiswa/reject", {
                    method: "POST",
                    body: JSON.stringify(data),
                  });
                  const show = await response.json();
                ...
                return (
                  <tr key={i}>
                    ...
                    {forMap.map((cs, i) => {
                      ...
                      return <th key={i}>{cb.data_locs[cs]}</th>;
                    })}
                    <td>
