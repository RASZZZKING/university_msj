            ...
            {datax
              ...
              .map((cb, i) => {
                ...
                const handleAccept = async (e) => {
                  ...
                  const response = await fetch("/api/v1/dataOrangtua/accept", {
                    method: "POST",
                ...
                const handleReject = async (e) => {
                  ...
                  const response = await fetch("/api/v1/dataOrangtua/reject", {
                    method: "POST",
                    body: JSON.stringify(data),
                  });
                  const show = await response.json();
                  ...
                  return (
                    <>
                      <tr>
                        ...
                        <td>{cb.nama}</td>
                        <th className="border-l">Ayah</th>
                        {keys.map((sb, i) => {
                          if (sb.split("_")[1] === "ayah")
                            return <td key={i}>{cb.data_locs[sb]}</td>;
                          else if (sb.split("_")[2] === "ayah")
                            return <td key={i}>{cb.data_locs[sb]}</td>;
                          return;
                        })}
                        <td>
                          <div className="flex justify-center">
                            <div
                              ...
                              ${
                                cb.data_locs.status === "Diterima"
                                  ? "btn-success text-success"
                                  : cb.data_locs.status === "Pending"
                      ...
                      <tr>
                        ...
                        <td ></td>
                        <th className="border-l">Ibu</th>
                        {keys.map((sb, i) => {
                          if (sb.split("_")[1] === "ibu")
                            return <td key={i}>{cb.data_locs[sb]}</td>;
                          else if (sb.split("_")[2] === "ibu")
                            return <td key={i}>{cb.data_locs[sb]}</td>;
                          return;
                        })}
                      </tr>
                      {cb.data_locs.nama_wali === "" ||
                      cb.data_locs.nama_wali === null ||
                      cb.data_locs.nama_wali === undefined ? null : (
                        <tr>
                          <th></th>
                          <td>
                          </td>
                          <td></td>
                          <th className="border-l" >Wali</th>
                          {keys.map((sb, i) => {
                            if (sb.split("_")[1] === "wali")
                              return <td key={i}>{cb.data_locs[sb]}</td>;
                            else if (sb.split("_")[2] === "wali")
                              return <td key={i}>{cb.data_locs[sb]}</td>;
                            return;
                          })}
                        </tr>
