...
const TabelPayment = ({ datax, field, forMap }) => {
  //outer
        ...
        <table className="table">
          ...
          <thead>
            <tr className="text-color-dark">
              ...
              {field.map((key, i) => {
                if (key === "id" || key === "id user" || key === "createdAt")
                  return;
                return (
            ...
            {datax
              ...
              .map((cb, i) => {
                const CobaAh = () => {
                  const showImg = (isi) => {
                    setIsVisible(true);
                    setUrl(`/bukti_pembayaran/${isi}`);
                  };
                  return (
                    <>
                      {cb.data_locs.map((en, inx) => {
                        const noUrut = `PPDB2024${
                          en.id_user < 10
                            ? `00${en.id_user}`
                            : en.id_user < 100
                            ? `0${en.id_user}`
                        ...
                        const handleAccept = async (e) => {
                          ...
                          if (show.status === 200)
                            return alert(`Data berhasil di acc`);
